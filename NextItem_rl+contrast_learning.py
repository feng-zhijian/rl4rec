...
class NextItNet(nn.Module):
    ...
    def forward(self, inputs, len_state, neg_items=None, next_true_item=None):
        ...
        if self.training:
            ...
            cl_loss = torch.mean(torch.mean(cl_loss, dim=-1))  # torch.Size([256]) => value
    ...
    for epoch in range(args.epoch):
        ...
        with tqdm(total=num_batches, desc='Training', ncols=80) as pbar:
            for j in range(num_batches):
                ...
                with torch.no_grad():
                    target_Qs, _, _ = target_QN(next_state, len_next_state, negative_item, next_true_item)
                    target_Qs_selector, _, _ = mainQN(next_state, len_next_state, negative_item, next_true_item)
                ...
                q_values, cl_loss, pred_next_item_emb = mainQN(state, len_state, negative_item, next_true_item)
