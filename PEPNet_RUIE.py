...
class PEPNet_RUIE(nn.Module):
    def __init__(self, hidden_size, item_num, scenario_num, 
        ...
        super(PEPNet_RUIE, self).__init__()
    ...
    def forward(self, inputs, len_state, neg_items=None, next_true_item=None, true_scenario=None, attention_state=None, attention_scenario=None, mask_array=None, scenario_ids=None, get_emb=False):
        ...
        personalized_embeddings = self.epnet(item_embeddings, scenario_embeddings)
        ...
        # PPNet
        x = mean_embeddings
        for idx,layer in enumerate(self.ppnet_dnn):
            if idx == 0:
                x = layer(x)
        ...
        q_input = torch.concat([pred_next_item_emb, last_attention_ret], dim=-1)
        ...
        # === 对比学习 ===
        cl_pred_next_item_emb = torch.concat([mean_embeddings, last_attention_ret], dim=-1)
...
if __name__ == '__main__':
    ...
    pepnet_ruie_model_1 = PEPNet_RUIE(
    ...
    pepnet_ruie_model_2 = PEPNet_RUIE(
        hidden_size=args.hidden_factor, 
        item_num=item_num, 
        scenario_num=scenario_num,
        state_size=state_size,
    ...
    print("initialize PEPNet RUIE model finished")

    optimizer_1 = torch.optim.Adam(pepnet_ruie_model_1.parameters(), lr=args.lr)
    optimizer_2 = torch.optim.Adam(pepnet_ruie_model_2.parameters(), lr=args.lr)
    ...
    for epoch in range(args.epoch):
        ...
        with tqdm(total=num_batches, desc='Training', ncols=80) as pbar:
            for j in range(num_batches):
                ...
                if pointer == 0:
                    mainQN, target_QN, optimizer = pepnet_ruie_model_1, pepnet_ruie_model_2, optimizer_1
                else:
                    mainQN, target_QN, optimizer = pepnet_ruie_model_2, pepnet_ruie_model_1, optimizer_2
