...
class HiNet_RUIE(nn.Module):
    def __init__(self, hidden_size, item_num, state_size, scenario_num, 
        ...
        super(HiNet_RUIE, self).__init__()
    ...
    def forward(self, inputs, len_state, neg_items=None, next_true_item=None, true_scenario=None, attention_state=None, attention_scenario=None, mask_array=None, scenario_ids=None, get_emb=False):
        ...
        HiNet_RUIE_prediction = task_predictions[0]

        logits = self.logits_layer(HiNet_RUIE_prediction)
        if get_emb:
            return logits

        nextitem_input = HiNet_RUIE_prediction
        ...
        q_input = torch.concat([HiNet_RUIE_prediction, last_attention_ret], dim=-1)
        ...
        # === 对比学习 ===
        cl_pred_next_item_emb = torch.concat([HiNet_RUIE_prediction, last_attention_ret], dim=-1)
...
if __name__ == '__main__':
    ...
    optimizer_1 = torch.optim.Adam(hinet_ruie_model_1.parameters(), lr=args.lr)
    optimizer_2 = torch.optim.Adam(hinet_ruie_model_2.parameters(), lr=args.lr)
    ...
    for epoch in range(args.epoch):
        ...
        with tqdm(total=num_batches, desc='Training', ncols=80) as pbar:
            for j in range(num_batches):
                ...
                if pointer == 0:
                    mainQN, target_QN, optimizer = hinet_ruie_model_1, hinet_ruie_model_2, optimizer_1
                else:
                    mainQN, target_QN, optimizer = hinet_ruie_model_2, hinet_ruie_model_1, optimizer_2
